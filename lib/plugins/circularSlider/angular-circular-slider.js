...
(function(window, angular, undefined) {
  ...
  var props = {
    ...
    defaults: {
	  ...
	  onTrySlide: angular.noop,
	  setCallback: angular.noop,
	  onSliderMouseUp: angular.noop,
    },
  ...
  var eventHandlers = (function() {
	...
	//  console.log(touches[0]);
     ...
     // if (touches.length > 1) return;
	  ...
	  e.clientX = clientX;
	  e.clientY = clientY;
	  //manually call involved handler
	  if(e.type === "mousemove" )
	  {
		mousemoveHanlder(e);
	  }
...
*/
    ...
    var translate = function(e) {
      var cursor = {
        x: e.clientX /*|| e.originalEvent.touches && e.originalEvent.touches[0].pageX*/,
        y: e.clientY /*|| e.originalEvent.touches && e.originalEvent.touches[0].pageY*/
      };

	  cursor.x -= cs.components.acs.offset().left;
	  cursor.y -= cs.components.acs.offset().top;
    ...
    var mousemoveHanlder = function(e) {
	 ...
	 cursor.x -= cs.components.acs.offset().left;
      ...
      var val = scope.clockwise ? d2v : (scope.max - d2v);
	  	...
	  	if (typeof scope.onTrySlide === 'function')
		{
			if(!scope.onTrySlide(val))
			{
				mouseDown = false;
				return;
			}	
    ...
    var mouseupHandler = function(e) {
	  ...
	  var scope = cs.components.scope;
	  if (typeof scope.onSliderMouseUp === 'function')
		{
			scope.onSliderMouseUp();
		}
    };
  ...
  function CircularSliderController($scope) {
	...
	$scope._specialVal = 3;
	$scope.text_value = "22*C";
	
	if (typeof $scope.setCallback === 'function')
          $scope.setCallback(
			//value callback
		  	function(value){
			...
			//color callback value
			function(colValue)
			{
				cs.components.acsValue.css({
					   'background-color': colValue
				});
			},
			
			//color callback track
			function(colTrack)
			{
				cs.components.acs.css({
					   'border-color': colTrack
				});
			},
			//set value
			function(value)
			{
				cs.funs.setValue(value);
				//$scope.value = $scope.$$value = value;
			}
		  );
    ...
    var transforms = {
      ...
      'function': {
        bindings: ['onSlide', 'onSlideEnd', 'setCallback'],
